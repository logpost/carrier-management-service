...
import { createDriverDTO, updateDriverDTO } from '../entities/dtos/driver.dto'
...
async function findDrivers(username: string, status: string | undefined): Promise<DriverInterface[]> {
	const accountRepository = AccountRepository.getInstance()
	try {
		const drivers = await accountRepository.findDriversByIdentifier({ username })
		if (status) {
			return drivers.filter((driver) => driver.status === parseInt(status, 10))
		}
		return drivers
	} catch (error) {
		console.log(error)
