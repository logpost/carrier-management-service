...
async function adminFindCarrierByIdentifier(identifier: identifierDTO): Promise<CarrierInterface> {
  try {
    ...
    const data = await accountRepository.adminFindCarrierByIdentifier(identifier)  
    if(data)
      return data
  } catch (error) {
...
async function deActivateCarrierAccount(req: deleteDTO): Promise<string> {
  ...
  if(hash){
    ...
    if(match){
      try {
        const shipprt_account = await accountRepository.findCarrierByIdentifier(identifier) as CarrierInterface
